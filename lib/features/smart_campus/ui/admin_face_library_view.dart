// `_LibraryRecord` 里的 `dorm` / `parentContact` / `photoBytes` 等字段是
// 给后端 `/app/school/v2/manager/faceList` 详情字段预留的，当前 demo
// 数据未填，analyzer 会提示 unused_element_parameter，这里整体忽略。
...
//         操作：紫色 "通过" / 红色 "驳回"。
//         数据接口待对接 `/app/school/v2/manager/faceList`，
//         审核接口待对接 `/app/school/v2/manager/faceAudit`。
// =============================================================================
...
extension _LibraryFilterX on _LibraryFilter {
  ...
  bool match(_LibraryStatus s) => switch (this) {
    _LibraryFilter.all => true,
    _LibraryFilter.effective => s == _LibraryStatus.effective,
    _LibraryFilter.pending => s == _LibraryStatus.pending,
    _LibraryFilter.rejected => s == _LibraryStatus.rejected,
  };
}
...
class _LibraryRecord {
  _LibraryRecord({
    ...
    this.gender = '女',
    this.dorm = '艺术苑·3 号楼·207',
    this.parentContact = '13800000000',
    this.submittedAt = '2026-04-21 16:38',
    this.auditedAt,
    this.auditedBy,
    this.photoBytes,
  });
  ...
  /// 来源（专业 / 设备 / 接入渠道等，对应 Figma 「来源」列）。
  final String source;

  /// 说明（对应 Figma 「说明」列，常见值：通过 / 驳回理由 / 备注）。
  String note;
  _LibraryStatus status;

  /// 详情弹窗里展示的扩展字段，接入后端后用真实数据替换。
  final String gender;
  final String dorm;
  ...
  /// 已采集的人脸缩略图字节流（接入接口前为空，UI 自动 fallback）。
...
class _AdminFaceLibraryViewState extends ConsumerState<AdminFaceLibraryView> {
  ...
  // TODO(api): 接入 `/app/school/v2/manager/faceList`（pending）。
  // 在接入前先用一份与 Figma 一致的 demo 数据驱动渲染 / 筛选 / 操作演示。
  late final List<_LibraryRecord> _records = <_LibraryRecord>[
    _LibraryRecord(
      id: 'r1',
      name: '钱丽红',
      studentNo: 'G094A1',
      ...
      source: '传播学',
      note: '通过',
      status: _LibraryStatus.effective,
      submittedAt: '2026-04-21 16:38',
      auditedAt: '2026-04-22 09:12',
      auditedBy: '李老师',
    ),
    _LibraryRecord(
      id: 'r2',
    ...
    _LibraryRecord(
      ...
      source: '中国画',
      note: '光线偏暗，请重新采集',
      status: _LibraryStatus.rejected,
      submittedAt: '2026-04-30 18:22',
      auditedAt: '2026-05-01 09:40',
      auditedBy: '王老师',
    ),
    _LibraryRecord(
      id: 'r4',
      ...
      note: '通过',
      status: _LibraryStatus.effective,
      submittedAt: '2026-04-19 14:55',
      auditedAt: '2026-04-19 15:30',
      auditedBy: '李老师',
    ),
    _LibraryRecord(
      id: 'r5',
      ...
      status: _LibraryStatus.effective,
      gender: '男',
      submittedAt: '2026-04-15 09:45',
      auditedAt: '2026-04-15 11:12',
      auditedBy: '李老师',
    ),
    _LibraryRecord(
      id: 'r6',
      ...
      note: '通过',
      status: _LibraryStatus.effective,
      submittedAt: '2026-04-12 17:08',
      auditedAt: '2026-04-13 08:50',
      auditedBy: '王老师',
    ),
    _LibraryRecord(
      id: 'r7',
      ...
      note: '通过',
      status: _LibraryStatus.effective,
      submittedAt: '2026-04-10 10:20',
      auditedAt: '2026-04-10 10:45',
      auditedBy: '李老师',
    ),
  ];
  ...
  List<_LibraryRecord> get _filteredRecords {
    final q = _query.trim().toLowerCase();
    return _records.where((r) {
      if (!_filter.match(r.status)) return false;
  ...
  void _onApproveRecord(_LibraryRecord r) {
    if (r.status == _LibraryStatus.effective) {
      AppToast.show(context, '该记录已生效');
      return;
    }
    setState(() {
      r.status = _LibraryStatus.effective;
      r.note = '通过';
      r.auditedAt = _nowFormatted();
      r.auditedBy = '我';
    });
    AppToast.show(context, '已通过「${r.name}」的人脸录入');
    // TODO(api): 接入审核接口，例如
    // `/app/school/v2/manager/faceAudit` { id, status: 'effective' }。
  }

  void _onRejectRecord(_LibraryRecord r) {
    if (r.status == _LibraryStatus.rejected) {
      AppToast.show(context, '该记录已驳回');
      return;
    }
    setState(() {
      r.status = _LibraryStatus.rejected;
      r.note = '驳回';
      r.auditedAt = _nowFormatted();
      r.auditedBy = '我';
    });
    ...
    // TODO(api): 接入审核接口，例如
    // `/app/school/v2/manager/faceAudit` { id, status: 'rejected' }。
  }
  ...
  static String _nowFormatted() {
    ...
    return '${n.year}-${pad(n.month)}-${pad(n.day)} '
        '${pad(n.hour)}:${pad(n.minute)}';
  }

  Future<void> _showRecordDetail(_LibraryRecord r) {
  ...
  Future<void> _loadStudents(String classId) async {
    ...
    try {
      ...
      final options = <_Option>[
        for (var i = 0; i < list.length; i++)
          (
            id: _pickString(list[i], ['id', 'studentId', 'userId'], 'srv-$i'),
            name: _pickString(list[i], [
              'realname',
              'realName',
              'nickname',
  ...
  Future<void> _onSubmit() async {
    ...
    // 当前后端「人脸录入」接口尚未提供，先停留在本地提交反馈。
    // 接入时改为：repo.faceSave({classId, studentId, photoBytes, photoName})。
...
class _StudentCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        // 32×32 圆形头像（接口接入前用首字默认头像；接入后改为
        // `DecorationImage(image: NetworkImage(record.avatarUrl), …)`）。
        Container(
          width: ui(32),
          height: ui(32),
          decoration: const BoxDecoration(
...
class _ActionCell extends StatelessWidget {
  const _ActionCell({required this.onApprove, required this.onReject});
