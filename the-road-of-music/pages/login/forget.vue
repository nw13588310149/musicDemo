...
<script setup>
import { ref } from 'vue';
import {sendSmsApi1 ,forget,updateToken} from '../../api/home.js'
...
const onSubmit = () => {
  forget({'mobile':username.value,'password':password.value,'smsCode':sms.value})
  .then(res =>{
	  if(res.code == 0){
		 showToast('重置密码成功！');
...
const sendSms = () => {
  ...
  sendSmsApi1(username.value).then((res) =>{
	  if(res.code == 0){
		  showToast('短信已发送！');
