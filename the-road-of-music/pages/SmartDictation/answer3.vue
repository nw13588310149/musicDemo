...
import dataAll  from './data2.js'
import dataAll2  from './data1.js'
import dataAll3 from './data3.js'
...
// 音符数据
const changeList = [
	'大三','小三','减三','增三','大六','小六','减六','大四六','小四六','减四六'
]
...
function playRandomNote() {
	...
	if(data?.data?.id){
		...
		const init = result[randomIndex];
		
		let num = dataAll.低音.s1.indexOf(init[0]);

		let str = init[1]+','+init[2];

		data.item.forEach(e =>{
			if(str == dataAll[e].s1[num]){
				answerTrue.value = e;
				allPlay.value.push(answerTrue.value)
			}
		})

		synth.triggerAttackRelease(init[0], '1n')
		synth.triggerAttackRelease(init[1], '1n')	
		synth.triggerAttackRelease(init[2], '1n')
		...
		try{
			...
			//确认有值
			let playN = [];
			dataA[items[randomIndex]].s1.forEach((e,i) =>{

				if(e != 'null'){
					playN.push({name:e,code:i})
				}
			})
			if(playN.length > 0){
				const randomIndex = Math.floor(Math.random() * playN.length);
				const playTwo = playN[randomIndex].name;
				const playOne = dataA.低音.s1[playN[randomIndex].code];
				
				let music = playTwo.split(',');
				music.push(playOne,playTwo);
				
				let a = true;
				// music.forEach(e =>{
				// 	if(dataAll2.根音.s1.indexOf(e) > dataAll2.根音.s2.indexOf(data.max)){
				// 		a = false;
				// 		playRandomNote()
				// 		return false;
				// 	}else if(data.isJb && jbarr.indexOf(e) != -1){
				// 		a = false;
				// 		playRandomNote()
				// 		return false;
				// 	}
				// })
				if(a){
					text.value.push(music)	
					music.forEach(e =>{
						synth.triggerAttackRelease(e, '1n')	
					})
					answerTrue.value = items[0];
					allPlay.value.push(answerTrue.value)
				}
			}else{
				playRandomNote();
		...
		// 2.0 获取低音
		playInit = [];
		dataA.低音.s1.forEach((e,i) =>{
			if(i>=dataA.低音.s2.indexOf(data.min)){
				// 是否过滤基本音
		...
		// 3.0获取对应音
		const playTwo = dataA[items[randomIndex]].s1[playInit[randomInit].code];
