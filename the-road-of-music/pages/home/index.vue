<template>
  <view class="right">
	  <!-- 轮播图 -->
	  <van-swipe :autoplay="4000" :duration="300" lazy-render class="swipe" ref="swipeRef" indicator-color='#fff'>
	  ...
	  <view style="display: flex;">
		  ...
		  <view class="btn_r">
		  	<!-- 上课通知 -->
		    <view class="notice1 kecheng">
				<!-- 日期 -->
				<view class="scroll_x">
					<view class="riqi">
						<view class="head_li" v-for="(item,index) in dataNav" :class="{'active':item.now}" @click="goKb">
							<view class="week">{{item.name}}</view>
							<view class="time">{{item.value}}</view>
							<view class="all">{{kbData[index]}}节课</view>
						</view>
					</view>

				</view>
				<view class="tongzhi">
					<view class="title">上课通知</view>
					<view class="tzBottom">
						<view v-if="!nextKC?.subject" style="font-size: 12px;color: #999;text-align: center;height: 50px;display: flex;align-items: center;justify-content: center;">暂无排课</view>
						<view class="one" v-if="nextKC?.subject">
							<view class="date">{{nextKC?.date?.substring(5,nextKC?.date?.length)}}</view>
							<view class="time">{{nextKC?.timeBegin?.substring(0,5)}} - {{nextKC?.timeEnd?.substring(0,5)}}</view>
							<view class="kc" @click="goKb">{{nextKC?.subject?.name}}课</view>
						</view>
						<view class="two" v-if="nextKC?.subject">
							<van-image
								class="head"
							  width="20"
							  height="20"
							  :src="nextKC?.teacher?.headUrl"
							/>
							<view class="text">授课教师：{{nextKC?.teacher?.realname || nextKC?.teacher?.nickname}}</view>
							<view class="time">{{syDate}}后开课</view>
						</view>
    ...
    <view class="title_new">
      <view class="latest" style="height: 23px;"><text style="position: absolute;;z-index: 9;">最新</text></view>
...
import { bannerList ,homeLatestInfo , homeLearningProgress,getMyInfo,nextSchoolCourse,courseList,classList} from '../../api/home.js'
...
homeLearningProgress({"province": "甘肃"}).then(res =>{
...
if(!uni.getStorageSync('checkStatus')){
	...
	{name:"试题",icon:"/static/assets/homeIcon4.png",url:'answerQuestions'},
	{name:"刷题",icon:"/static/assets/homeIcon5.png",url:'camp'},
	{name:"资讯",icon:"/static/assets/homeIcon7.png",url:'consultation'},]
	...
	{name:"试题",icon:"/static/assets/homeIcon4.png",url:'answerQuestions'},
	{name:"刷题",icon:"/static/assets/homeIcon5.png",url:'camp'},
	{name:"模考",icon:"/static/assets/homeIcon6.png",url:'mock'},
	{name:"资讯",icon:"/static/assets/homeIcon7.png",url:'consultation'},
	{name:"商城",icon:"/static/assets/homeIcon8.png",url:'AiSong'},
]
}
