			...
			<view class="scroll" v-if="list">
				...
				<view class="li" v-for="item in list" :key="item" v-if="isShot">
					...
					<van-grid :gutter="12" :column-num="3" :border='false'>
						    <van-grid-item class="list_item" v-for="(a,value) in item.list" :key="value" @click="tap(a)">
				...
				<van-grid :gutter="12" :column-num="3" :border='false' v-if="!isShot">
					    <van-grid-item class="list_item" v-for="(a,value) in list" :key="value" @click="tap(a)">
							<view class="tip" v-if="isHide">{{a.vip==0?'免费':'会员'}}</view>
							<van-icon class="icon" name="/static/assets/icon_yf2.jpg" />
							<view class="item-top" :style="{top:a.shortText2?'-7px':'0px'}">
							  {{a.title}}
...
function tap(value){
	...
	router.push({ name: 'music', state: { id: value.id } });
}
...
const getMenulist = async () => {
  try {
    const result = await postMenulist(3);
	...
	if(uni.getStorageSync('firstMenu')){
		navItems.value.forEach(e =>{
			if(e.id == uni.getStorageSync('firstMenu')){
				onSelectNavItem(e)
				if(e.id == 12){
					if(uni.getStorageSync('secondMenu')){
						activeSecond.value = uni.getStorageSync('secondMenu');
						getlist()
					}
				}
			}
